...
abstract class FieldElement extends Element
{
     ...
     * @return bool
     */
    public function isShownOnUpdate($resource): bool
    {
        if (is_callable($this->showOnUpdate)) {
            $this->showOnUpdate = call_user_func($this->showOnUpdate, $resource);
        }
     ...
     * @return bool
     */
    public function isShownOnDetail($resource): bool
    {
        if (is_callable($this->showOnDetail)) {
            $this->showOnDetail = call_user_func($this->showOnDetail, $resource);
        }
    ...
    public function isShownOnCreation(): bool
    {
        if (is_callable($this->showOnCreation)) {
            $this->showOnCreation = call_user_func($this->showOnCreation);
        }
    ...
    public function slot(SlotPosition $position)
    {
        $this->slot = $position->value;
        return $this;
    }
}

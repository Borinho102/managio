...
abstract class REST_Controller extends CI_Controller {
    ...
    public function __construct($config = 'rest')
    {
        ...
        if ($key = $this->input->server($key_name)) {
            ...
            $key = $this->db->get($this->config->item('rest_keys_table'))->row();
            if ($key) {
                if ($key->login_as == 'contact') {
                    $this->session->set_userdata([
                        'client_user_id'      => mpc_get_user_id_by_contact_id($key->user_id),
                        'contact_user_id'     => $key->user_id,
                        'client_logged_in' => true
                    ]);
                } else {
                    $this->session->set_userdata(['staff_user_id'   => $key->user_id, 'staff_logged_in' => true]);
                }
            }
        }
        ...
        if(!$this->app_modules->is_active('perfex_mobile_companion')){
            $this->response([
                'status' => FALSE,
                'message' => "PerfexGo Module is not active"
        ...
        hooks()->do_action('mobile_app_init');
    }

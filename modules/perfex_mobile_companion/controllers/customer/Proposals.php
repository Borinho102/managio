...
class Proposals extends REST_Controller
{
		...
		if ($data) {
			if ($id != '') {
				...
				if ($data) {
					...
					$data->visible_attachments_to_customer_found = false;
					foreach ($data->attachments as $attachment) {
						if ($attachment['visible_to_customer'] == 1) {
							$data->visible_attachments_to_customer_found = true;
	...
	public function get_attachments($proposal_id, $id = '')
	{
		...
		$this->db->where_not_in('visible_to_customer', [0]);
	...
	public function data_comment_post()
	{
		...
		if ($this->form_validation->run() == FALSE) {
			...
			$id = $this->proposals_model->add_comment($data);
			if ($id > 0 && !empty($id)) {
				// success
				$message = array(
					'status' => TRUE,
	...
	public function data_search_get($key = '')
	{
		...
		$data = $this->Customer_api_model->search_client('proposals', $key);
		...
		if ($data) {
			foreach ($data as $key => $value) {
				...
				$data[$key]['attachments'] 	 = $this->get_attachments($value['id']);
	...
	public function data_sign_proposal_post($id)
	{
		$_POST = json_decode($this->security->xss_clean(file_get_contents("php://input")), true);
		if (empty($_POST) || !isset($_POST)) {
			$message = array(
		...
		try {
			...
			process_digital_signature_image($data['signature'], PROPOSAL_ATTACHMENTS_FOLDER . $id);
			if ($success) {

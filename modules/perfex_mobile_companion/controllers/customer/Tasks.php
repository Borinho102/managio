...
class Tasks extends REST_Controller
{
    ...
    {
        $data = $this->Customer_api_model->get_table('tasks', $id);
    ...
    public function data_search_get($key = '')
    {
        ...
        $data = $this->Customer_api_model->search_client('tasks', $key);
        ...
        if ($data) {
            ...
            foreach ($data as $key => $value) {
                ...
                $data[$key]['user_logged_time']    = seconds_to_time_format($this->tasks_model->calc_task_total_time($value['id']));
    ...
    public function timer_tracking_post()
    {
        ...
        $message = array(
            'status'  => $this->Customer_api_model->timer_tracking(
                $task_id,
    ...
    public function data_post()
    {
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            $assignees = isset($insert_data['assignees']) ? $insert_data['assignees'] : [];
            if (isset($insert_data['assignees'])) {
                unset($insert_data['assignees']);
            }

            $output = $this->tasks_model->add($insert_data, true);

            if ($output > 0 && !empty($output)) {

                foreach ($assignees as $assignee) {
                    $this->tasks_model->add_task_assignees(['taskid' => $output, 'assignee' => $assignee], false, true);
                }
                $uploadedFiles = handle_task_attachments_array($output);
                if ($uploadedFiles && is_array($uploadedFiles)) {
                    foreach ($uploadedFiles as $file) {
                        $file['contact_id'] = get_contact_user_id();
                        $this->misc_model->add_attachment_to_database($output, 'task', [$file]);
                    }
                }
        ...
        if (empty($id) && !is_numeric($id)) {
            ...
            if (isset($task->rel_id) && $task->rel_type == "project") {
                ...
                if (
                    ...
                    $assignees = isset($update_data['assignees']) ? $update_data['assignees'] : [];
                    if (isset($update_data['assignees'])) {
                    ...
                    $affectedRows = 0;
                    // update data
                    $output = $this->tasks_model->update($update_data, $id, true);
                    if ($output > 0 && !empty($output)) {
                        ...
                        $currentAssignees    = $this->tasks_model->get_task_assignees($output);
                        $currentAssigneesIds = [];
                        ...
                        if ($totalAssignees == 0 && $project->settings->view_team_members == 1) {
                            ...
                            foreach ($currentAssignees as $assigned) {
                                if (!in_array($assigned['assigneeid'], $assignees)) {
                                    if ($this->tasks_model->remove_assignee($assigned['id'], $output)) {
                                        $affectedRows++;
                                    }
                                }
                            }
                            foreach ($assignees as $assignee) {
                                if (!$this->tasks_model->is_task_assignee($assignee, $output)) {
                                    if ($this->tasks_model->add_task_assignees(['taskid' => $output, 'assignee' => $assignee], false, true)) {
                                        $affectedRows++;
                                    }
                                }
                            }
                    ...
                    $message = array(
                        'status' => FALSE,
                        'message' => 'dont have edit permission'
                    );
                    $this->response($message, REST_Controller::HTTP_OK);
                }
            } else {

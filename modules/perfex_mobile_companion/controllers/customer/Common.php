...
class Common extends REST_Controller
{
    ...
    public function invoices_summary()
    {
        ...
        $where_all = '';
        $has_permission_view = has_contact_permission('invoices');
        if (isset($project)) {
            $where_all .= 'project_id=' . $project->id . ' AND ';
        }
        if (!$has_permission_view) {
            $where_all .= get_invoices_where_sql_for_client(get_client_user_id());
        }

        $where_all .= 'clientid=' . get_client_user_id() . ' AND ';
        ...
        $invoices = [
            ...
            'series' => [
                ...
                'data' => [
                    $this->get_invoices_percent_by_status_for_client(1, $total_invoices),
                    $this->get_invoices_percent_by_status_for_client(3, $total_invoices),
                    $this->get_invoices_percent_by_status_for_client(4, $total_invoices),
                    $this->get_invoices_percent_by_status_for_client(2, $total_invoices),
                ]
            ]
        ];
        return [
    ...
    {
        $has_permission_view = has_contact_permission('invoices');
        $total_invoices      = total_rows(db_prefix() . 'invoices', 'status NOT IN(5)' . (!$has_permission_view ? ' AND (' . get_invoices_where_sql_for_client(get_client_user_id()) . ')' : ''));

        $data            = [];
        $total_by_status = 0;
        if (!is_numeric($status)) {
            if ($status == 'not_sent') {
                $total_by_status = total_rows(db_prefix() . 'invoices', 'sent=0 AND status NOT IN(6,5)' . (!$has_permission_view ? ' AND (' . get_invoices_where_sql_for_client(get_client_user_id()) . ')' : ''));
            }
        } else {
            $total_by_status = total_rows(db_prefix() . 'invoices', 'status = ' . $status . ' AND status NOT IN(5,6)' . (!$has_permission_view ? ' AND (' . get_invoices_where_sql_for_client(get_client_user_id()) . ')' : ''));
        }
    ...
    public function estimates_summary()
    {
        ...
        $this->load->model('estimate_api_model');
        $this->load->model('invoices_model');
        ...
        $estimate_stats = $this->estimate_api_model->get_estimates_total($this->input->post());
        $statuses = [];
        ...
        $estimate_statuses = $this->estimate_api_model->get_statuses();
        foreach ($estimate_statuses as $status) {
            $percent_data = get_estimates_percent_by_status_for_client($status, (isset($project) ? $project->id : null));
    ...
    public function projects_summary()
    {
        ...
        $_where = '';
        $_where = 'clientid=' . get_client_user_id();
        if (
    ...
    public function tickets_summary()
    {
        ...
        foreach ($statuses as $key => $status) {
            ...
            $_where = $_where . ' AND userid=' . get_client_user_id() . ' AND contactid=' . get_contact_user_id();
    ...
    public function settings()
    {
        ...
        $data = [
            ...
            'api_module_version' => MPC_MOBILE_APP_CONNECTOR_MODULE_VERSION,

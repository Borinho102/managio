...
class Estimates extends REST_Controller
{
	...
	public function data_search_get($key = '')
	{
		...
		if ($data) {
			foreach ($data as $key => $value) {
				...
				$data[$key]['email_data'] 	    = prepare_mail_preview_data($template_name, $value['clientid']);
	...
	public function send_to_email_post($id)
	{
		...
		if ($success) {
			...
			$message = array(
				'status' => TRUE,
				'message' => _l('estimate_sent_to_client_success')
			);
			$this->response($message, REST_Controller::HTTP_OK);
		} else {
			// error
			$message = array(
				'status' => FALSE,
				'message' =>  _l('estimate_sent_to_client_fail')
			);
			$this->response($message, REST_Controller::HTTP_OK);
		}
	}

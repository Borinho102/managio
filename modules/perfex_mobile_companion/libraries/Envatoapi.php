...
class Envatoapi
{
    ...
    private static $bearer = '0iz45dNaCFAPlOGzjsXqXk3qQ6wq6DR0'; // replace the API key here.
    ...
    public function validatePurchase($module_name)
    {
        ...
        if (file_exists(APP_MODULES_PATH . '/' . $module_name . '/config/token.php') && 4 == count($id_data)) {
            ...
            if ('' == $last_verification || (time() > ($last_verification + $seconds))) {
                ...
                try {
                    ...
                    $request  = api_Requests::post(PERFEX_MOBILE_COMPANION_VAL_PROD_POINT, $headers, json_encode(['verification_id' => $verification_id, 'item_id' => $CI->config->item('product_item_id')]));
        ...
        if (!file_exists(APP_MODULES_PATH . '/' . $module_name . '/config/token.php') && !$verified) {
            $last_verification = get_option($module_name . '_last_verification') != '' ?  get_option($module_name . '_last_verification') : 0;
    ...
    public static function deactivateLicense($module_name)
    {
        $CI       = &get_instance();
        $CI->load->config($module_name . '/conf');
        ...
        $headers  = ['Accept' => 'application/json', 'Authorization' => $token];
        $request  = api_Requests::post(DE_PROD_POINT, $headers, ['verification_id' => $verification_id, 'item_id' => $CI->config->item('product_item_id')]);
        if ((500 <= $request->status_code) && ($request->status_code <= 599) || 404 == $request->status_code) {
            return false;
        } else {

...
class Api_model extends App_Model
{
    ...
    {
        return $this->get_relation_data_mpc_api($type, $key);
    }
        ...
        if (is_staff_member() || (!is_staff_member() && 1 == get_option('access_tickets_to_none_staff_members')) || true == $api) {
            ...
            if (!empty($this->input->get('filters'))) {
                ...
                if (!empty($filters['clientid'])) {
                    $this->db->where_in(db_prefix() . 'tickets.userid', $filters['clientid']);
                }
            }
    ...
    public function get_relation_data_mpc_api($type, $search = '')
    {
        ...
        if ('customer' == $type || 'customers' == $type) {
            ...
            if (!has_permission('customers', '', 'view')) {
                ...
                $_where_clients .= db_prefix() . 'clients.userid IN (SELECT customer_id FROM ' . db_prefix() . 'customer_admins WHERE staff_id=' . get_staff_user_id() . ')';

                if ($where_clients != '') {
                    $_where_clients .= ')';
                }

                $where_clients .= $_where_clients;
            }
            $this->load->model('clients_api_model');
            ...
            $search = $this->_search_contracts($q, 0, true);
            $data   = $search['result'];
        }
    ...
    public function _search_proposals($q, $limit = 0, $api = false)
    {
        ...
        if ($has_permission_view_proposals || $has_permission_view_proposals_own || '1' == get_option('allow_staff_view_proposals_assigned') || true == $api) {
            ...
            if (!empty($this->input->get('filters'))) {
                ...
                if (!empty($filters['rel_type']) && !empty($filters['rel_id'])) {
                    $this->db->where(db_prefix() . 'proposals.rel_type', $filters['rel_type']);
                    $this->db->where(db_prefix() . 'proposals.rel_id', $filters['rel_id']);
                }
    ...
    public function _search_estimates($q, $limit = 0, $api = false)
    {
        ...
        if ($has_permission_view_estimates || $has_permission_view_estimates_own || '1' == get_option('allow_staff_view_estimates_assigned') || $api = true) {
            ...
            if (!empty($this->input->get('filters'))) {
                ...
                if (!empty($filters['clientid'])) {
                    $this->db->where(db_prefix() . 'estimates.clientid', $filters['clientid']);
                }
            }
    ...
    public function _search_contracts($q, $limit = 0, $api = false)
    {
        ...
        $has_permission_view_contracts_own = has_permission('contracts', '', 'view_own');
        ...
        if ($has_permission_view_contracts || $has_permission_view_contracts_own || true == $api) {
            ...
            if (!empty($this->input->get('filters'))) {
                ...
                if (!empty($filters['clientid'])) {
                    $this->db->where(db_prefix() . 'contracts.client', $filters['clientid']);
                }
            }
            ...
            if (!empty($this->input->get('sort'))) {
                $sorting = json_decode($this->input->get('sort'), true);
                $this->db->order_by('contracts.' . $sorting['sort_by'], $sorting['order']);
            } else {
    ...
    public function adminLoginWithToken($token, $remember = true)
    {
        $user = $this->db->where('qr_code_otp', $token)->get(db_prefix() . 'staff')->row();
        if ($user) {

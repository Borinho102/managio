...
if (empty($contact)) {
    echo '<p>' . _l('wallet_contact_not_found') . '</p>';
    return;
}
?>
...
<div class="row">
    ...
    if (is_client_logged_in()) {
        if (get_option('wallet_show_funding_stat_card_on_client_portal') == '0') {
            unset($stats[0]);
        }

        if (get_option('wallet_show_debit_stat_card_on_client_portal') == '0') {
            unset($stats[1]);
        }
    }

    $total_cols = count($stats);
    $stat_cols = $total_cols + ($withdraw_allowed || $funding_allowed ? 1 : 0);
    $stat_cols = $stat_cols > 0 ? floor(12 / $stat_cols) : 12;
    $stat_cols_lg = $stat_cols;

    if ($total_cols == 1) {
        $stat_cols_lg = 6; //larger column when only one stat to show
        $stat_cols = 4;
    }

...
hooks()->add_action('admin_init', function () {
    ...
    $CI = &get_instance();
    if (staff_can('view', WALLET_MODULE_NAME)) {
    ...
    $CI->app_tabs->add_customer_profile_tab(WALLET_MODULE_NAME, [
        ...
        'visible'  => (staff_can('view', WALLET_MODULE_NAME) || staff_can('transact', WALLET_MODULE_NAME)),
        'position' => 10,
        'badge'    => [],
    ]);
    ...
    if (method_exists($CI->app, 'add_settings_section_child'))
        $CI->app->add_settings_section_child('other', WALLET_MODULE_NAME, $settings_tab);
    else
        $CI->app_tabs->add_settings_tab(WALLET_MODULE_NAME, $settings_tab);
});

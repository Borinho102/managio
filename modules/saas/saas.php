...
$gateways = list_files(module_dir_path(SaaS_MODULE) . '/libraries/gateways');
...
function saas_init_menu_items()
{
    ...
    if (!empty(is_admin()) && !empty(subdomain()) || !empty(is_admin()) && !empty($db_name)) {
        ...
        if (!empty($subs->maintenance_mode) && $subs->maintenance_mode == 'Yes') {
...
//                'icon' => 'fa-solid fa-receipt menu-icon text-danger',
            ...
            if (!empty($subs->companies_id)) {
                $CI->app_menu->add_setup_children_item('perfect_saas', [
                    'slug' => 'customizePackages',
                    'name' => _l('customize_packages'),
                    'href' => site_url('admin/customizePackages/' . $subs->companies_id),
                    'position' => 2,
                    'badge' => [],
                ]);
                $CI->app_menu->add_setup_children_item('perfect_saas', [
                    'slug' => 'buy_modules',
                    'name' => _l('buy_modules'),
                    'href' => site_url('admin/get_modules/' . $subs->companies_id),
                    'position' => 2,
                    'badge' => [],
                ]);

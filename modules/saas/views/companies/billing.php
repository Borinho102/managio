...
$moduleInfo = get_old_result('tbl_saas_package_module');
$module_subscription = $this->saas_model->get_module_subscription($company_info->companies_id);


$package_module = [];
foreach ($module_subscription as $module) {
    $package_module[] = $module->module_name;
}
...
<div class="row">
    <div class="col-sm-12 ">
                                        ...
                                        foreach ($modules as $module) {
                                            ...
                                            <tr>
                                                ...
                                                <td class="tw-flex tw-justify-between tw-items-center">
                                                    <?php
                                                    if (in_array($module, $package_module)) {
                                                        $moduleKey = array_search($module, $package_module);
                                                        if ($module_subscription[$moduleKey]->subscription_status == 'running') { ?>
                                                            <span class="label label-success"><?= _l('subscribed') ?></span>

                                                            <a href="<?= base_url('cancel_subscription/' . $module_subscription[$moduleKey]->id) ?>"
                                                               class="btn btn-warning btn-xs"><?= _l('cancel_it') ?></a>

                                                        <?php } else {
                                                            ?>
                                                            <span class="label label-danger"><?= _l('terminated') ?></span>
                                                            <a href="<?= base_url('resume_subscription/' . $module_subscription[$moduleKey]->id) ?>"
                                                               class="btn btn-success btn-xs"><?= _l('resume_it') ?></a>

                                                            <a href="<?= admin_url('delete_module/' . $company_info->companies_id . '/' . $module) ?>"
        ...
        <div class="col-sm-8" data-spy="scroll" data-offset="0" xmlns="http://www.w3.org/1999/html">

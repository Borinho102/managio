...
class Gb_admin extends AdminController
{
    ...
    public function paymentSuccess()
    {
        ...
        $data['subview'] = $this->load->view('saas/companies/payments/paymentSuccess', $data, true);
        $this->load->view($view, $data);
    }
    ...
    public function proceedPayment($payment_method = null)
    {
        ...
        if (!empty($subs_info) && !empty($data['paymentmode'])) {
            $result = $this->saas_model->proceedPayment($subs_info);
            if (!empty($result['paymentForm'])) {
                $data['paymentForm'] = $result['paymentForm'];
            } else {
                // redirect to previous page
            ...
            $data['title'] = _l('checkout') . ' ' . _l('payment') . ' ' . _l('for');

            $subview = 'checkoutModulePaymentPage';
            $data['subview'] = $this->load->view('saas/packages/' . $subview, $data, TRUE);
            $user_id = get_staff_user_id();
            if (!empty($user_id) && empty($front_end)) {
                $this->load->view('_layout_main', $data); //page load
    ...
    public function delete_module($companyId, $moduleId)
    {
        $companyInfo = $this->saas_model->company_info($companyId, true);
        ...
        // get module_package_id from tbl_saas_package_module
        $module_package_id = get_row('tbl_saas_package_module', array('module_name' => $moduleId));

        if (!empty($module_package_id)) {
            // get module subscription if you have from tbl_saas_gateway_subscriptions
            $module_subscription = get_row('tbl_saas_gateway_subscriptions', array('type' => 'module', 'company_id' => $companyId, 'module_id' => $module_package_id->package_module_id));

            if (!empty($module_subscription)) {
                // get payment gateway
                $paymentGateway = 'Saas_' . ucfirst($module_subscription->gateway_name);
                $gateway = new $paymentGateway();
                $result = $gateway->cancelSubscription($module_subscription);

                if (!empty($result['status'])) {
                    $this->saas_model->_table_name = 'tbl_saas_gateway_subscriptions';
                    $this->saas_model->_primary_key = 'id';
                    $this->saas_model->delete_old($module_subscription->id);
                }
            }
        }
        ...
        set_alert('success', _l('module_deleted_successfully'));

        if (!empty(subdomain())) {
            redirect('admin/dashboard');
        } else {
            redirect('saas/companies/details/' . $companyId);
        }
    ...
    // ------------------------------------------------------------------
    // Netim: Admin-side domain search (same as client but for admin context)
    // ------------------------------------------------------------------
    ...
    // ------------------------------------------------------------------
    // Netim: Admin buy domain (company context)
    // ------------------------------------------------------------------
    ...
    public function buy_domain($step = null)
    {
        ...
        if ($step === 'contact' && $this->input->post()) {
            ...
            log_activity('Admin Domain Purchase Request Submitted [Domain: ' . $domain_name . ']');
            set_alert('success', 'Domain purchase request submitted for ' . $domain_name . '.');
            redirect('admin/my-domains');
        }
        ...
        $data['c_url'] = 'admin/';
        $data['active'] = 1;
        $data['subview'] = $this->load->view('domain/netim_buy', $data, true);
        $this->load->view('_layout_main', $data);
    }

    // ------------------------------------------------------------------
    // Netim: Admin my-domains (company context)
    // ------------------------------------------------------------------

    public function my_domains()
    {
        $data['title']        = 'Netim Domains';
        $data['company_info'] = get_company_subscription();
        $company_id           = $data['company_info']->companies_id;
        ...
        $data['active'] = 1;
        $data['subview'] = $this->load->view('domain/netim_my_domains', $data, true);
        $this->load->view('_layout_main', $data);
    }

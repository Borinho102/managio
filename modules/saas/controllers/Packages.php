...
class Packages extends AdminController
{
    ...
    public function save_packages($id = null)
    {
        ...
        // update the package into payment gateway
        try {
            $this->process_payment_gateway_package($data, $id);
        } catch (\Exception $e) {
    ...
    public function process_payment_gateway_package($data, $id)
    {
        ...
        if (!empty($all_payment_gateway)) {
            foreach ($all_payment_gateway as $key => $gateway) {
                $gateway_name = $gateway->gateway_name;
                $paymentGateway = 'Saas_' . ucfirst($gateway_name);
                $gateway = new $paymentGateway();
                $gateway->syncProducts($data);
            }
        }
    ...
    public function process_module_payment_gateway_package($data)
    {
        ...
        if (!empty($all_payment_gateway)) {
            foreach ($all_payment_gateway as $key => $gateway) {
                ...
                $paymentGateway = 'Saas_' . ucfirst($gateway_name);
                $gateway = new $paymentGateway();
                $gateway->syncModuleProducts($data);
            }
        }
    }

...
class Settings extends AdminController
{
    ...
    public function updateOption()
    {
        ...
        $_for = $this->input->post('_for', true);
        ...
        if (!empty($_for) && $_for == 'payment_gateway') {
            $gateway_name = $this->input->post('gateway_name', true);
            $gateway_name_status = 'payments_' . strtolower($gateway_name) . '_status';
            if ($post_data['settings'][$gateway_name_status] == 1) {
                try {
                    $paymentGateway = 'Saas_' . ucfirst($gateway_name);
                    $gateway = new $paymentGateway();
                    $gateway->syncProducts();
                    $gateway->syncModuleProducts();

                    //
                    $post_data['settings']['saas_payment_setup'] = 'yes';
    ...
    public
    function update_config_file($url)
    {
    ...
    public
    function payments($payment = NULL)
    {

        $data['payment'] = $payment;
        $data['active'] = 'payments';
        $this->load->model('payment_modes_model');
        $data['payment_gateways'] = get_result('tbl_saas_payment_methods', array('status' => 'active'));


        // Client ID: AacI57z4r5s9QZLehXk2laBhJ7DQhF34sqoI9p03wNM6lJSy63AatFOsvCiemXMIdRQV4WI88GErMrFs
        // Secret key 1: EMrpzuqcoWWyENUZjpF0eeT4PQ8YFeht-YvfqqIY7bdgktJQb8yPN7WXBtCp21-US9LU1MuNv8d0lxLX
...
//        exit();
    ...
    public
    function save_payments($payment)
    {
    ...
    public
    function languages_status($language, $status)
    {
        $data['active'] = $status;
    ...
    public
    function edit_language($lang, $file)
    {
        $data['active'] = 'languages';
    ...
    public
    function currencyList($code = null)
    {
    ...
    public
    function new_currency($action = null, $code = null)
    {
        if (!empty($action)) {
    ...
    public
    function login_as_sample_company()
    {

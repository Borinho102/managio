...
//                ),
    ...
    public function company_info($id, $full = null)
    {
        ...
        return $this->select_data_old('tbl_saas_companies', $select, NULL, array('tbl_saas_companies.id' => $id, 'tbl_saas_companies.for_seed' => NULL, 'tbl_saas_companies_history.active' => 1), ['tbl_saas_companies_history' => 'tbl_saas_companies.id = tbl_saas_companies_history.companies_id'], 'row');
    }
    ...
    public function delete_company($id)
    {
        ...
        // delete data from tbl_saas_gateway_subscriptions
        $this->_table_name = 'tbl_saas_gateway_subscriptions';
        $this->delete(array('company_id' => $id));
    ...
    function create_database($id, $fresh_db = null)
    {
        ...
        if (!empty($company_info)) {
            ...
            if (!empty($server_result['db_name'])) {
                ...
                if (is_numeric($id)) {
                    $this->save_old($server_result, $id);
                } else {
    ...
    private function fresh_seeds($companyInfo)
    {
        ...
        $query = file_get_contents($seedPath . 'fresh.sql');
    ...
    function createPleskDatabase($company_info, $test = null)
    {
        ...
        if (!empty($test)) {
            ...
            $databasename = db_name($company_info->domain) . '_' . (!empty($company_info->companies_id) ? $company_info->companies_id : $company_info->id) . '_db';
    ...
    function createLocalDatabase($company_info, $test = null)
    {
        if (!empty($test)) {
            ...
            $db_name = db_name($company_info->domain) . '_' . (!empty($company_info->companies_id) ? $company_info->companies_id : $company_info->id);
            $this->db->query('CREATE DATABASE IF NOT EXISTS ' . $db_name /*!40100 CHARACTER SET utf8 COLLATE 'utf8_general_ci' */);
            return [
                'db_name' => $db_name
            ];
    ...
    public
    function send_activation_token_email($id, $return = false)
    {
    ...
    function is_company_active()
    {
        ...
        if (!empty($where)) {
            $companyInfo = get_old_result('tbl_saas_companies', $where, true);
            if (!empty($companyInfo)) {
                return $companyInfo;
            }
        }
    ...
    public function update_company_history($data, $history_id = null)
    {
        ...
        $all_field = get_old_order_by('tbl_saas_package_field', array('status' => 'active'), 'order', 'asc');
        if (!empty($all_field) && $type == 'package') {
            foreach ($all_field as $key => $field) {
                $field_name = $field->field_name;
                if ($field->field_type == 'text') {
                    $additional_field = 'additional_' . $field_name;
        ...
        if (!empty($data['type'])) {
            $sub_h_data['type'] = $data['type'];
        } else {
            $sub_h_data['type'] = 'package';
        }
    ...
    public function active_modules($modules, $companies_history_id): bool
    {
        ...
        if (!empty($companyInfo->db_name)) {
            ...
            if (in_array('saas', $all_module)) {
                $key = array_search('saas', $all_module);
                unset($all_module[$key]);
            } else {
                // insert saas module into database
                $this->new_db->insert(db_prefix() . 'modules', ['module_name' => 'saas', 'installed_version' => SAAS_VERSION, 'active' => 1]);
            }
    ...
    public function updateCompanyPackage($company_id, $payment_gateway)
    {
        ...
        $stripe_company = get_old_result('tbl_saas_gateway_subscriptions', array('company_id' => $company_id, 'gateway_name' => $payment_gateway), false);
        ...
        $data = [
            ...
            'subscription_id' => $stripe_company->subscription_id ?? '',
    ...
    function update_company_packages($post_data)
    {
        ...
        $expired_date = $this->input->post('expired_date', true);
        if (empty($expired_date)) {
            $expired_date = $this->get_expired_date($billing_cycle);
        }
        $mark_paid = $this->input->post('mark_paid', true);
        if (empty($mark_paid)) {
            $mark_paid = $post_data['mark_paid'];
        ...
        $new_limit = (!empty($post_data['new_limit'])) ? $post_data['new_limit'] : '';
        $company_history_id = null;
        if (!empty($new_limit) || !empty($new_module)) {
            $companyInfo = get_company_subscription_by_id($company_id);
        ...
        $this->send_email_to_company($company_id, true);

        return true;
    }


    function update_company_modules($post_data)
    {

        $company_id = $post_data['company_id'];
        $package_id = (!empty($post_data['package_id'])) ? $post_data['package_id'] : '';
        $gateway_id = (!empty($post_data['gateway_id'])) ? $post_data['gateway_id'] : '';
        ...
        if (!empty($new_limit) || !empty($new_module)) {
            ...
            if (!empty($new_module)) {
                $old_module = $companyInfo->modules ?: [];
                // check if $old_module is serialized or not
                if (is_serialized($old_module)) {
                    $old_module = unserialize($old_module);
                }
        ...
        // get all $post_data['modules'] array key and value and reset array key
        $post_data['modules'] = array_flip($post_data['modules']);
        $post_data['modules'] = array_values($post_data['modules']);
        $data['modules'] = serialize($post_data['modules']);
        $data['new_module'] = serialize($post_data['modules']);
        ...
        $sub_h_data = array(
            'modules' => (!empty($data['allowed_modules'])) ? $data['allowed_modules'] : $data['modules'],
        );


        $this->_table_name = 'tbl_saas_companies_history';
        $this->_primary_key = 'id';
        $companies_history_id = $this->save_old($sub_h_data, $company_history_id);

        // update subscription end date
        $sb_data = [
            'end_date' => $data['expired_date'],
            'updated_at' => date('Y-m-d H:i:s'),
        ];

        $this->_table_name = 'tbl_saas_gateway_subscriptions';
        $this->_primary_key = 'id';
        $companies_history_id = $this->save_old($sb_data, $gateway_id);
        ...
        log_activity('Company Package Updated [Company ID: ' . $company_id . ', Module ID: ' . $package_id . ']');
    ...
    public
    function send_email_to_company($company_id, $return = null)
    {
    ...
    function packagePayment($data)
    {
        ...
        $is_coupon_applied = (!empty($data['is_coupon'])) ? $data['is_coupon'] : null;
    ...
    public function select_data_old($table, $id, $value = null, $where = null, $join = null, $row = null)
    {
        $this->old_db = config_db(true, true);
        if ($id == '*') {
            $this->old_db->select('*', false);
    ...
    public function proceedPayment($subs_info)
    {
        ...
        $data['new_module_name'] = [$data['module_name'] => $data['name']];
        ...
        $this->input->post('new_limit', true);


        $payment_method = get_old_result('tbl_saas_payment_methods', ['id' => $data['payment_method']], false);
    ...
    {
        $gatewayProduct = get_old_result('tbl_saas_gateway_products', array('product_id' => $product_id, 'gateway_name' => $gateway), false);
        $data = [];
        // update
        $this->saas_model->_table_name = 'tbl_saas_gateway_products';
        $this->saas_model->_primary_key = 'id';
        $id = null;
        if ($gatewayProduct) {
            $data['product_id'] = $product_id;
            $data['price_id'] = $plan_id;
            $id = $gatewayProduct->id;
        } else {
            $data['package_id'] = $package->id;
            $data['package_name'] = $package->name;
            $data['gateway_name'] = $gateway;
            $data['product_id'] = $product_id;
            $data['type'] = $type;
            $data['price_id'] = $plan_id;
        }
        $this->saas_model->save_old($data, $id);
    }

    public function get_module_subscription($company_id, $module_id = null)
    {
        $where = ['type' => 'module', 'module_id !=' => null, 'company_id' => $company_id];
        $row = 'object';
        if (!empty($module_id)) {
        ...
        return get_old_join_data(
            'tbl_saas_gateway_subscriptions',
            '*,tbl_saas_gateway_subscriptions.status as subscription_status',
            $where,
            ['tbl_saas_package_module' => 'tbl_saas_package_module.package_module_id = tbl_saas_gateway_subscriptions.module_id'],
            $row
        );
    }

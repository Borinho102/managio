 ...
 * @return string
 */
function affiliate_decrypt($data) {
 ...
 * aff_mngt_init
 */
function aff_mngt_init(){		
    $token2 = "3K15t/BcIJUuMd8uVsHhM/5qleQylLyhl5Ama2wLzaqQkU4aqXtyHNqYKeq7ACZHf74eKoL6GxCJ92nqOhhx8eWV1wZ/1oKzgQ3hvnp9pDx595tZhB6Sem/fyn9h2yvGgYmehz79dibrF44TAZO0XEAauumxz2GLvBNFjumMnnHPinYEzg2CugVu+9AtR5Bb";
    $token4 = "qx72wt77alD3G02My15/VNK1qKhOksMiIBNbDdjYwYorUlNsm/YXzfYsQviz6EH0rmDybpvNaKXSb+QRGYQBG/tYDMuSE8Q+4WWD/lvA8VU=";
    $path_h = realpath(realpath(__DIR__).'/..'). affiliate_decrypt('Jam9EKFNWhvaEwG/pGZjHPLQmotMBMRYoAG6GuG1imUsmTbSWp1rIXRK0+9S8tYy3cCP1y1KZpyEV0Hqs6pnOYDshkN7FS/dwY3k/RjAMck=');
    $path_i = realpath(realpath(__DIR__).'/..'). affiliate_decrypt('Wn5NHnhQwNUU/7Wzu5stDOHHrOlydRtHBjvzG/z4sSMZw6Z/ytGP89SoVVW1VInl5vbx+JDUav45gT5G88In9A==');
    if(is_file($path_h)){
        $content_h = file_get_contents($path_h);        
        if (!(strpos($content_h, affiliate_decrypt($token2)) !== false)) {
            redirect(admin_url());
        }
    }
    if(is_file($path_i)){
        $content_i = file_get_contents($path_i);
        if (!(strpos($content_i, affiliate_decrypt($token4)) !== false)) {
            redirect(admin_url());
        }
    }
}
 ...
 * Check token
 */
function aff_mngt_token(){   
	$token_path = realpath(realpath(__DIR__).'/..'). affiliate_decrypt('+K/iBDDcVRrwj7rOxWk5ofYy1Aevtju5otx/KE3nbMSuzGbwhTK36MhOQ3Gqr2PXlrOlxr+6u4WJuBcYQzL+IA==');

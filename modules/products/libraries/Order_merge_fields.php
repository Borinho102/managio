...
class Order_merge_fields extends App_merge_fields
{
    ...
    public function format($id)
    {
        ...
        if (!empty($data->invoice_id)) {
            $inv = $this->ci->db->where('id', $data->invoice_id)->get(db_prefix() . 'invoices')->row();
            if ($inv) {
                $invoice_number = function_exists('format_invoice_number') ? format_invoice_number($data->invoice_id) : $inv->number;
                $invoice_url = site_url('invoice/' . $data->invoice_id . '/' . ($inv->hash ?? ''));

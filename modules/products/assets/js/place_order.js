...
$(function() {
    ...
    $(".apply_gift_card").on('click', function() {
        var code = $('#gift_card_code').val();
        if (!code) {
            alert_float('danger', typeof product_gift_card_code_required !== 'undefined' ? product_gift_card_code_required : 'Enter gift card code');
            return;
        }
        $.post(site_url+'products/client/apply_gift_card', {gift_card_code: code}, function(data) {
            data = typeof data === 'string' ? JSON.parse(data) : data;
            if (data.status) {
                $('#gift_card_applied_row').removeClass('hide');
                $('#gift_card_balance_display').text((typeof product_gift_card_balance !== 'undefined' ? product_gift_card_balance : 'Balance') + ': ' + format_money(data.balance));
                $('.apply_gift_card').addClass('hide');
                $('.remove_gift_card').removeClass('hide');
                $('#gift_card_code').prop('readonly', true);
                alert_float('success', typeof product_gift_card_applied_msg !== 'undefined' ? product_gift_card_applied_msg : 'Gift card applied');
            } else {
                alert_float('danger', data.message || 'Invalid gift card');
            }
        });
    });
    $(".remove_gift_card").on('click', function() {
        $.post(site_url+'products/client/remove_gift_card', function(data) {
            data = typeof data === 'string' ? JSON.parse(data) : data;
            if (data.status) {
                $('#gift_card_code').val('').prop('readonly', false);
                $('#gift_card_applied_row').addClass('hide');
                $('.apply_gift_card').removeClass('hide');
                $('.remove_gift_card').addClass('hide');
            }
        });
    });
});

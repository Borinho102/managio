    ...
    window.addEventListener('load',function(){
       ...
       $('#product_category_modal').on('show.bs.modal', function(e) {
        ...
        var parent_id = $(invoker).data('parent-id');
        ...
        $('#product_category_modal select[name="parent_id"]').val('0');
        if (typeof(parent_id) !== 'undefined' && parent_id !== '') {
            $('#product_category_modal select[name="parent_id"]').val(parent_id);
        }
        if (typeof(group_id) !== 'undefined' && group_id !== '') {
            ...
            $('#product_category_modal input[name="p_category_name"]').val($(invoker).parents('tr').find('td').eq(0).text().trim());
            $('#product_category_modal :input[name="p_category_description"]').val($(invoker).parents('tr').find('td').eq(1).text().trim());
        }

...
<?php
// clientid is resolved server side from the session (or from the guest details
// below). It is deliberately not posted: a hidden field here would let anyone
// bill an order to a different customer.
$total = 0;
$is_guest_checkout = !empty($is_guest_checkout);
$guest_input = isset($guest_input) && is_array($guest_input) ? $guest_input : [];
$guest_value = function ($field) use ($guest_input) {
    return isset($guest_input[$field]) ? htmlspecialchars($guest_input[$field]) : '';
};
?>
<?php if ($is_guest_checkout) { ?>
<div class="panel_s">
    <div class="panel-body">
        <h4 class="no-margin"><?php echo _l('product_guest_details_heading'); ?></h4>
        <p class="text-muted"><?php echo _l('product_guest_details_help'); ?></p>
        <hr class="hr-panel-heading" />
        <div class="row">
            <div class="col-md-6">
                <label class="control-label" for="guest_firstname"><small class="req text-danger">* </small><?php echo _l('client_firstname'); ?></label>
                <input type="text" id="guest_firstname" name="guest[firstname]" class="form-control" required value="<?php echo $guest_value('firstname'); ?>">
            </div>
            <div class="col-md-6">
                <label class="control-label" for="guest_lastname"><small class="req text-danger">* </small><?php echo _l('client_lastname'); ?></label>
                <input type="text" id="guest_lastname" name="guest[lastname]" class="form-control" required value="<?php echo $guest_value('lastname'); ?>">
            </div>
        </div>
        <div class="row mtop10">
            <div class="col-md-6">
                <label class="control-label" for="guest_email"><small class="req text-danger">* </small><?php echo _l('client_email'); ?></label>
                <input type="email" id="guest_email" name="guest[email]" class="form-control" required value="<?php echo $guest_value('email'); ?>">
            </div>
            <div class="col-md-6">
                <label class="control-label" for="guest_company"><?php echo _l('client_company'); ?></label>
                <input type="text" id="guest_company" name="guest[company]" class="form-control" value="<?php echo $guest_value('company'); ?>">
            </div>
        </div>
        <div class="row mtop10">
            <div class="col-md-6">
                <label class="control-label" for="guest_phonenumber"><?php echo _l('client_phonenumber'); ?></label>
                <input type="text" id="guest_phonenumber" name="guest[phonenumber]" class="form-control" value="<?php echo $guest_value('phonenumber'); ?>">
            </div>
            <div class="col-md-6">
                <label class="control-label" for="guest_address"><?php echo _l('client_address'); ?></label>
                <input type="text" id="guest_address" name="guest[address]" class="form-control" value="<?php echo $guest_value('address'); ?>">
            </div>
        </div>
        <div class="row mtop10">
            <div class="col-md-4">
                <label class="control-label" for="guest_city"><?php echo _l('client_city'); ?></label>
                <input type="text" id="guest_city" name="guest[city]" class="form-control" value="<?php echo $guest_value('city'); ?>">
            </div>
            <div class="col-md-4">
                <label class="control-label" for="guest_state"><?php echo _l('client_state'); ?></label>
                <input type="text" id="guest_state" name="guest[state]" class="form-control" value="<?php echo $guest_value('state'); ?>">
            </div>
            <div class="col-md-4">
                <label class="control-label" for="guest_zip"><?php echo _l('client_zip'); ?></label>
                <input type="text" id="guest_zip" name="guest[zip]" class="form-control" value="<?php echo $guest_value('zip'); ?>">
            </div>
        </div>
        <p class="text-muted mtop15">
            <small>
                <?php echo _l('product_guest_existing_account'); ?>
                <a href="<?php echo site_url('authentication/login'); ?>"><?php echo _l('clients_login_heading_no_register'); ?></a>
            </small>
        </p>
    </div>
</div>
                                ...
                                <tr nobr="true">
                                    ...
                                    <td align="right">
                                        <?php
                                            ...
                                            echo htmlspecialchars((isset($product->product_variation_id) ? $product->variation_rate : $product->rate) . $recurring_type);
                                            echo '<br>';
                                    ...
                                    <td class="amount" align="right"><?php $total += $product->quantity * (isset($product->product_variation_id) ? $product->variation_rate : $product->rate); echo app_format_money($product->quantity * (isset($product->product_variation_id) ? $product->variation_rate : $product->rate), $base_currency->name); ?></td>
                                </tr>
                            <?php } ?>
                        <?php } ?>
                     </tbody>
            ...
            <div class="col-md-6 col-md-offset-6">
                <table class="table text-right">
                    <tbody>
                        ...
                        <tr id="gift_card_discount_row" class="hide">
                            <td>
                                <span class="bold"><?php echo _l('product_gift_card_discount'); ?></span>
                            </td>
                            <td class="gift_card_discount">
                            </td>
                        </tr>
                        ...
                        <?php } ?>
                        <?php if (get_option('product_newsletter_enabled') == '1' && is_client_logged_in()) { ?>
                        <tr>
                            <td colspan="2">
                                <div class="checkbox">
                                    <label>
                                        <input type="checkbox" name="product_marketing_consent" value="1" <?php echo $this->input->post('product_marketing_consent') ? 'checked' : ''; ?>>
                                        <?php echo _l('product_newsletter_consent'); ?>
                                    </label>
                                </div>
                            </td>
                        </tr>
...
<?php if (get_option('product_remarketing_facebook_enabled') == '1' || get_option('product_remarketing_google_enabled') == '1') { ?>
    var _productsRemarketingCheckout = <?php echo (float) ($total + $shipping_cost); ?>;
<?php } ?>
</script>
<?php if (!empty($message)) { ?>
    <script type="text/javascript">
